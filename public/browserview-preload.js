/**
 * BBZCloud - BrowserView Preload Script
 * 
 * This preload script runs in each BrowserView and provides:
 * - Keyboard shortcut capture and forwarding
 * - Credential injection capabilities
 * - Communication bridge with main process
 * - Debug information collection
 * 
 * Unlike the WebView preload script, this runs in BrowserViews which have
 * better isolation and security characteristics.
 * 
...
// Get BrowserView ID from command line arguments
const browserViewId = process.argv.find(arg => arg.startsWith('--browserview-id='))?.split('=')[1] || 'unknown';

console.log(`[BrowserView Preload] Loading for BrowserView: ${browserViewId}`);
...
// Expose protected methods for BrowserView communication
contextBridge.exposeInMainWorld('electronBrowserView', {
  // Get the BrowserView ID
  getBrowserViewId: () => browserViewId,
  ...
  send: (channel, data) => {
    ...
    if (validChannels.includes(channel)) {
      ipcRenderer.send(channel, { ...data, browserViewId });
    }
  },
  ...
  on: (channel, callback) => {
    ...
    if (validChannels.includes(channel)) {
      ipcRenderer.on(channel, (event, data) => {
        if (data.browserViewId === browserViewId || !data.browserViewId) {
          callback(data);
        }
      });
    }
...
// ============================================================================
// KEYBOARD SHORTCUT CAPTURE SYSTEM FOR BROWSERVIEWS
// ============================================================================

/**
 * BrowserView-specific keyboard shortcuts
 * These are captured before websites can handle them
 */
const BROWSERVIEW_SHORTCUTS = [
  ...
  // Global app shortcuts that should work from BrowserViews
...
/**
 * Main keyboard event handler for BrowserView shortcuts
 */
function handleKeyboardShortcut(event) {
  ...
  if (window.electronBrowserView) {
    window.electronBrowserView.send('browserview-message', {
  ...
  const matchedShortcut = BROWSERVIEW_SHORTCUTS.find(shortcut => 
  ...
  console.log(`[BrowserView Preload] Matched shortcut: ${matchedShortcut.action} for ${shortcutString}`);
  ...
  if (isInInputField && !isAllowedInInput) {
    console.log(`[BrowserView Preload] Shortcut blocked - in input field: ${matchedShortcut.action}`);
    return;
  }
  ...
  console.log(`[BrowserView Preload] Sending shortcut to main process: ${matchedShortcut.action}`);
  
  // Send the shortcut to the main process
  if (window.electronBrowserView) {
    window.electronBrowserView.send('keyboard-shortcut', {
...
const CredentialInjector = {
  ...
  async injectWebUntisCredentials(email, password) {
    try {
      console.log('[BrowserView Preload] Injecting WebUntis credentials');
      ...
      console.error('[BrowserView Preload] WebUntis credential injection failed:', error);
      return false;
    }
  },
  ...
  async injectSchulCloudCredentials(email, password) {
    try {
      console.log('[BrowserView Preload] Injecting SchulCloud credentials');
      ...
      console.error('[BrowserView Preload] SchulCloud credential injection failed:', error);
      return false;
    }
  },
  ...
  async injectOfficeCredentials(email, password) {
    try {
      console.log('[BrowserView Preload] Injecting Office credentials');
      ...
      console.error('[BrowserView Preload] Office credential injection failed:', error);
      return false;
    }
  },
  ...
  async injectMoodleCredentials(email, password) {
    try {
      console.log('[BrowserView Preload] Injecting Moodle credentials');
      ...
      console.error('[BrowserView Preload] Moodle credential injection failed:', error);
      return false;
    }
  },
...
if (window.electronBrowserView) {
  window.electronBrowserView.on('inject-credentials', async (data) => {
    const { service, credentials } = data;
    
    console.log(`[BrowserView Preload] Received credential injection request for: ${service}`);
    ...
    switch (service.toLowerCase()) {
      ...
      default:
        console.warn(`[BrowserView Preload] Unknown service for credential injection: ${service}`);
    }
    
    // Send response back to main process
    window.electronBrowserView.send('credential-response', {
      service,
      success,
      timestamp: Date.now()
    });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', handleKeyboardShortcut, true);
    console.log('[BrowserView Preload] Keyboard listener re-attached after DOM ready');
  });
}

// Debug logging
console.log(`[BrowserView Preload] Script loaded for BrowserView: ${browserViewId}`);
console.log('[BrowserView Preload] Keyboard shortcut capture and credential injection ready');
