...
async function getCredentials(service, account) {
  try {
    return await credentialService.getPassword(service, account);
  } catch (error) {
...
ipcMain.handle('save-credentials', async (event, { service, account, password }) => {
  try {
    await credentialService.setPassword(service, account, password);
    return { success: true };
  } catch (error) {
...
async function getEncryptionPassword() {
  try {
    const password = await credentialService.getPassword('bbzcloud', 'password');
    if (!password) {
