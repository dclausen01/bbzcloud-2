...
class DatabaseService {
    ...
    async initializeDatabase() {
        try {
            ...
            try {
                ...
                console.log('Using better-sqlite3 for database operations');
                ...
                console.warn('better-sqlite3 not available, falling back to sqlite3:', betterSqliteError.message);
                ...
                // Enable foreign keys and verify database for sqlite3
                await new Promise((resolve, reject) => {
                    this.db.serialize(() => {
                        // Enable foreign keys
    ...
    async reencryptData(oldPassword, newPassword) {
        return this.withConnection(async () => {
            ...
            if (this.useBetterSqlite) {
                // better-sqlite3 implementation with transactions
                const transaction = this.db.transaction(() => {
                    try {
                        // 1. Reencrypt todos
                        const todos = this.db.prepare('SELECT * FROM todos').all();
                        const updateTodo = this.db.prepare('UPDATE todos SET text = ? WHERE id = ?');
                        ...
                        for (const todo of todos) {
                            ...
                            updateTodo.run(newEncryptedText, todo.id);
                        }

                        // 2. Reencrypt secure documents
                        const docs = this.db.prepare('SELECT * FROM secure_documents').all();
                        const updateDoc = this.db.prepare('UPDATE secure_documents SET content = ? WHERE id = ?');
                        ...
                        for (const doc of docs) {
                            try {
                                ...
                                updateDoc.run(newEncrypted, doc.id);
                            } catch (error) {
                ...
                // sqlite3 implementation (original code)
    ...
    async closeConnection() {
        ...
        if (this.useBetterSqlite) {
            // better-sqlite3 - synchronous close
            this.db.close();
            this.isConnected = false;
        } else {
            // sqlite3 - asynchronous close (original code)
    ...
    async ensureInitialized() {
        ...
        if (this.useBetterSqlite) {
            // better-sqlite3 doesn't need connection management
            if (!this.isConnected || !this.db) {
                await this.initializeDatabase();
            }
        } else {
            // sqlite3 connection management (original code)
    ...
    async withConnection(operation) {
        try {
            ...
            if (!this.useBetterSqlite) {
                // Only manage connections for sqlite3
                if (this.closeTimeout) {
                    clearTimeout(this.closeTimeout);
                }
                if (this.isConnected) {
            ...
            if (!this.useBetterSqlite && this.isConnected) {
    ...
    // Encryption/Decryption helpers (unchanged)
    encrypt(data) {
                ...
                settings = await new Promise((resolve, reject) => {
                    this.db.get(
                        ...
                        (err, row) => {
                            ...
                            let parsedSettings = {};
                            try {
                                parsedSettings = row ? JSON.parse(row.value) : {};
                            } catch (error) {
                                console.error('Error parsing settings:', error);
                            }
    ...
    async saveTodoState(todoState) {
        return this.withConnection(async () => {
            ...
            if (this.useBetterSqlite) {
                // better-sqlite3 implementation with transaction
                const transaction = this.db.transaction(() => {
                    // Clear existing data
        ...
        return this.withConnection(async () => {
            ...
            if (this.useBetterSqlite) {
                ...
                try {
                    ...
                    // Get todo settings
                    const settingsRow = this.db.prepare('SELECT value FROM settings WHERE id = ?').get('todo_settings');
                    if (settingsRow) {
                        try {
    ...
    async getCustomApps() {
        return this.withConnection(async () => {
            if (this.useBetterSqlite) {
                // better-sqlite3 implementation
                const apps = this.db.prepare('SELECT * FROM custom_apps ORDER BY title').all();
                return apps.map(app => ({
                    id: app.id,
                    title: app.title,
                    url: app.url,
        ...
        return this.withConnection(async () => {
            ...
            if (this.useBetterSqlite) {
                ...
                const stmt = this.db.prepare(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                `);
                stmt.run(document.id, document.name, document.size, document.date, encrypted, timestamp);
                return true;
            } else {
                // sqlite3 implementation
                return new Promise((resolve, reject) => {
    ...
    async getSecureDocuments() {
        return this.withConnection(async () => {
            if (this.useBetterSqlite) {
                // better-sqlite3 implementation
                const docs = this.db.prepare('SELECT id, name, size, date FROM secure_documents ORDER BY date DESC').all();
                return docs || [];
            } else {
                // sqlite3 implementation
                return new Promise((resolve, reject) => {
